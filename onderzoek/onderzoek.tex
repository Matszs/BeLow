...
\begin{abstract}
In this paper our team set out to explicate the gap between the demand for monitoring technology for the elderly, and the actual grasp upon the market. Initially a continuation of the Healthy Life Lamp project by Sven Erik Haitjema, we leaned more in the direction of a portable living lab rather than one central monitoring device. After interviews with the respondents (senior citizens still living independently or semi-independently) refuted the notion that all elderly feel uncomfortable when making use of monitoring technology, notably wearables, the main focus of the research became not only developing the product, but also how to make it fitting for this widely underestimated audience. After conduction of a focus group at living community Stedeborgh, Grootebroek, results showed that the target audience value their privacy and autonomy above all, meaning they do not want to be recorded, and that they want to be the ones in control of the system. The prototype was designed with scalability in mind, meaning that sensor 'nodes' can be added or removed to the user's liking, and can easily be customized with the sensors of choice thanks to the grove shield.  Several of the respondents from the focus group were provided with two nodes and a gateway, which were then deployed in the homes of the respondents. The experiment proved that the system was capable of acquiring data and transmitting it using LoRa. \footnote{Low power Radio}  The system was also deployed in the homes of respondents aged 21-27 to compare the acquired data. This, however, showed little to no difference since no incidents occurred during the research. After a week of monitoring the responses were positive.
\end{abstract}
...
% misschien inderdaad dat negatieve achterwege laten? zoals mats ook al zei
A drawback to this transmission method would be the limitations on message size and message frequency. This means that it is impossible  to send continuous streams of data, or large quantities of data; such as graphical camera recordings. 
