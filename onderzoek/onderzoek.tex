...
The aging of the population of the Netherlands causes a growing need for  solutions that enable the elderly to continue living independently for longer without putting their health and wellbeing at risk. Despite the wide range of products available, very few catch on. This leads to the question :"how can we develop an accessible standard with which to add a layer of intelligence to the living environment?"  We found an imbalance between supply and demand by conducting interviews and a focus group. These were conducted among the elderly residents of Stedeborgh, a living community in Grootebroek, the Netherlands. The imbalance results from a wrong view on what the target audience wants and needs. Many consider their privacy invaded by existing systems. They feel that the many systems that make use of camera's watch their every move, not to mention their guests'. Respondents feel that they do not have enough control over what is monitored and where the data ends up. We designed a prototype that is scalable and transparent. This means that the user gets to decide what is monitored and when, and where the gathered data ends up; who has access to it and who doesn't. to add ambient intelligence to the living spaces of the target audience. The prototype was then deployed in the homes of the respondents. the results showed that a need exists for ambient environments that work together with, and provide feedback to the user. This symbiosis allows for prolonged autonomy and peace of mind, without the user feeling scrutinized.
...
The ultimate goal when developing the prototype, is for solutions that are currently present in the form of expensive and closed commercial technologies, can be molded into the shape of a community based platform. During this proces we will have to be constantly aware of the fact that specialized care or technologies will always remain necessary in many cases. However, over the past years more and more platforms of increasing technological prowess such as Arduino, Raspberry Pi et cetera have become available to the public. These platforms come with an array of sensors capable of measuring everything from sound, to temperature, to light. These sensors are relatively accurate, but often do lack the reliability to truly gain knowledge from individual measurements. For this, machine based learning can offer the solution. With the rise of for example neural networks, new layers can be added to the data that is gathered by not only looking at the data alone, but also at trends and deviations specific for said user. Studies have explored the possibilities of linking the motor score of AMPS\footnote{Assessment of motor and process skills} to the sensor data using linear regression\cite{robben2012grandma}. Through machine based learning correlations, patterns and deviations from these two can be detected and eventually add valuable layers to the gathered data. The platform offers a method to gather large amounts of data. With the help of specialists, machine based learning and continuation of research we hope to eventually shape this data into information, knowledge, and finally wisdom (Figure \ref{fig:dikw}). Once the wisdom layer is reached, it becomes possible to not only respond to calamities, but prevent them altogether.\cite{rowley2007wisdom} This way, the final product will unburden the caregivers and the care budget, and  be shaped after the wishes of the target audience  while still preventing human error through its self-learning capabilities.
\begin{figure}
\centering
...
% =============================================================================
The prototype was focused around scalability, meaning that sensors could quickly be added and removed without the need for tools. The main controller used, the Sodaq Autonomo, and its compatible Grove shield enabled this. The Grove modules created by Sodaq made it possible to entertain a plug-and-play approach for developers, so that the prototype could we adjusted whenever needed. 
\begin{figure}
